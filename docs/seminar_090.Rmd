...
Для этого нам прежде всего понадобятся данные в хитром формате `russia.geojson`,
...
```{r}
set.seed(777)
rus@data <- mutate(rus@data, num = rnorm(n = 83, mean = 100, sd = 20))
```
...
rus %>%
  ...
  addPolygons(stroke = FALSE, fillColor = ~pal(num), # раскрашиваем области
              label = ~paste0(name, ': ', formatC(num))) %>% # добавляем подписи
  addLegend(pal = pal, values = ~num) %>% # добавляем легенду
...
Данные с добавленными случайными числами можно сохранить командой `geojson_write()` из пакета `geojsonio`.
...
```{r}
# data_old <- rus@data %>% ___(name, num, cartodb_id)
# glimpse(data_old)
```
...
Подменяем старые данные внутри нашей карты отредактированными `new_data`:
...
#   leaflet() %>%
#   addTiles() %>% # инициализируем карту
#   addPolygons(stroke = FALSE, fillColor = ~pal(___), # раскрашиваем области
#               label = ~paste0(___, ': ', formatC(___))) %>% # добавляем подписи
#   ___(pal = pal, values = ~num) %>% # добавляем легенду
...
htmlreg(list(grun_re, grun_fe, grun_fd), custom.model.names = model_names)
```
