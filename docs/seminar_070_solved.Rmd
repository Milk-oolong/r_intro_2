...
```{r}
def_lmodel_glm <- glm(data = def_train, default ~ ., family = binomial(link = 'logit'))
summary(def_lmodel_glm)
```
...
```{r}
def_margins <- margins(def_lmodel_glm)
summary(def_margins)

pred_def_vis <- ggpredict(def_lmodel_glm, terms = c('balance', 'student'))
plot(pred_def_vis)
```
...
```{r}
def_tree <- train(data = def_train, default ~ ., method = 'rpart2')
rpart.plot(def_tree$finalModel)
summary(def_tree)
```
...
```{r}
def_tree_pred <- mutate(def_test,
  yhat = predict(def_tree, def_test))
confusionMatrix(def_tree_pred$yhat, def_tree_pred$y)
```
